...
namespace AutoLua.Core.Extensions.Tools.Strings
{
    /// <summary>
    /// 数制格式化器
    /// </summary>
    public class NumberFormater
    {
        ...
        public static string ToChineseNumber(double num)
        {
            ...
            {
                result += ToInt(x.Substring(0, x.IndexOf(".", StringComparison.Ordinal))) + "点" + x.Substring(x.IndexOf(".", StringComparison.Ordinal) + 1).Aggregate("", (current, t) => current + ToNum(t));
            }
            else
            {
                result += ToInt(x);
