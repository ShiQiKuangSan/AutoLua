...
namespace AutoLua.Core.AutoAccessibility.Accessibility.Event
{
    ...
    public class AccessibilityEventNotifications : IAccessibilityEvent
    {
        ...
        public void Event(AccessibilityService autoAccessibilityService, AccessibilityEvent e)
        {
            ...
            {
                //通知栏操作
                AutoGlobal.Instance?.NotificationMonitorEvent?.OnNotification(Notification.Create(notification, e.PackageName));
            }
            else
            {
                //当前 autolua 的包名
                var appPackageName = AppUtils.AppContext?.PackageName ?? string.Empty;
